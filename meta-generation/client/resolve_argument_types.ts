...
import resolveVariableType from "./resolve_variable_type";
...
export default function resolveArgumentTypes(
  ...
  while (!exhausted) {
    ...
    if (!is_callback) {
      ...
      const type = resolveVariableType(name);

      result.push({ name, type, optional });
    } else {
      const type = resolveRequiredCallbackType(class_name, argument_text);
