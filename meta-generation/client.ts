...
import resolveArgumentTypes from "./client/resolve_argument_types";
import resolveRequiredCallbackType from "./client/resolve_required_callback_type";
...
import { TypeDefinition, isDefinitionEnum } from "./client/definition_map";
...
function buildMetaFile() {
  const chapters = readDocs(config, "../../docs");
  ...
  for (const section of all_sections) {
    ...
    if (heading.includes("function(")) {
      ...
      type = resolveRequiredCallbackType(table_name, heading);
    } else {
      // ### `table.property`
  ...
  fs.writeFileSync("hubos-client.d.lua", output.join("\n"));
}
...
function outputFunctionDefinition(
  ...
  // inject arguments
  const argumentTypes = resolveArgumentTypes(table_name, argument_group);
